...
const registerFlow = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx, { flowDynamic, gotoFlow, endFlow }) => {
    ...
    if (isUser) {
      ...
      return gotoFlow(appointmentsFlow); // 🚀 salta directo al flujo de citas
    }
    await flowDynamic("📋 Antes de agendar, necesito registrarte rápidamente.");
    // aquí continúa hacia los .addAnswer siguientes SOLO si no está registrado
  })
  .addAnswer(`¿Cuál es tu *nombre completo*?`, { capture: true }, async (ctx, ctxFn) => {
    await ctxFn.state.update({ name: ctx.body });
    await ctxFn.flowDynamic(`Gracias ${ctx.body} 🙌`);
  })
  .addAnswer(`¿Cuál es la *placa* de tu vehículo? (Ej: ABC123)`, { capture: true }, async (ctx, ctxFn) => {
  ...
  .addAnswer(`¿Cuál es la *marca y modelo* de tu vehículo? (Ej: Toyota Corolla)`, { capture: true }, async (ctx, ctxFn) => {
    await ctxFn.state.update({ brandModel: ctx.body });
  })
  .addAnswer(`¿Cuál es el *tipo de combustible y transmisión*? (Ej: Gasolina Automático)`, { capture: true }, async (ctx, ctxFn) => {
