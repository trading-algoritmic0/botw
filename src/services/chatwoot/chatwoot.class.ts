...
class ChatwootClass {
  ...
  getOpenConversation = async (dataIn: { inbox_id: string; contact_id: string }) => {
    try {
      ...
      const openConversation = data.payload.find((c: any) => c.inbox_id === Number(dataIn.inbox_id) && c.status === "open");
      return openConversation || null;
    } catch (error) {
      console.error(`[Error getOpenConversation]`, error.message);
      return null;
    }
  };
  ...
  checkAndSetCustomAttribute = async () => {
    try {
      const existingAttributes: any[] = await this.getAttributes();
      const attributeExists = existingAttributes.some(
        (attr: any) => attr.attribute_key === "phone_number"
      );
  ...
  findOrCreateConversation = async (dataIn: { inbox_id: string; contact_id: string; phone_number: string }) => {
    try {
      ...
      if (!conversation.length) return await this.createConversation(dataIn);
